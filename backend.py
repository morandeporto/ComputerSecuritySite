...
def register():
    ...
    if request.method == "POST":
        ...
        if check_if_user_exists_using_email(new_email):
            flash(
                "Email already exists! please use different email or login to your account."
            )
...
def dashboard():
    ...
    if client_data == ["False"]:
        return render_template(
            "dashboard.html", username=username, client_data=client_data
        )
    if client_data != []:
...
def set_new_pwd():
        ...
        if user_data:
            ...
            if isinstance(old_password, str):
                if not compare_to_current_password(user_data, old_password):
                    flash(
                        "The old password you inserted does not match the current used password.\nPlease try again"
                    )
    ...
    return render_template(
        "set_new_pwd.html", emailReset=request.args.get("emailReset")
    )
